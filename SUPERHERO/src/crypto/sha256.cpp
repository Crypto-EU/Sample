...
void write_le64(uint8_t* out, uint64_t value) {
    ...
    write_le32(out + 4, static_cast<uint32_t>(value >> 32));
}
...
uint64_t read_le64(const uint8_t* in) {
    return static_cast<uint64_t>(read_le32(in)) |
           (static_cast<uint64_t>(read_le32(in + 4)) << 32);
}
...
void Sha256::write(std::span<const uint8_t> data) {
...
void Sha256::transform() {
    uint32_t w[64];
    for (int i = 0; i < 16; ++i) {
        const uint8_t* p = buffer_.data() + i * 4;
...
void Sha256::finalize(std::array<uint8_t, kDigestSize>& out) {
...
std::array<uint8_t, Sha256::kDigestSize> Sha256::hash(std::span<const uint8_t> data) {
    Sha256 ctx;
    ctx.write(data);
...
std::array<uint8_t, Sha256::kDigestSize> Sha256::hash256d(std::span<const uint8_t> data) {
